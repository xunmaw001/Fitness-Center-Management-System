                ...
                <fieldset class="search-container" style="text-align: center;">
                    <form class="layui-form" lay-filter="myForm">
                      ...
                      <div class="layui-inline" style="margin-bottom: 10px;">
                          <label class="layui-form-label">优惠信息名称</label>
                          <div class="layui-input-inline">
                              <input type="text" name="youhuixinxiName" id="youhuixinxiName" placeholder="优惠信息名称" autocomplete="off" class="layui-input">
                          </div>
                      </div>
         
                      <div class="layui-inline" style="margin-bottom: 10px;">
                          <label class="layui-form-label">优惠信息类型</label>
                          <div class="layui-input-inline">
                              <select name="youhuixinxiTypes" id="youhuixinxiTypes" lay-filter="youhuixinxiTypes">
                                  <option value="">--请选择--</option>
                                  <option v-for="item in youhuixinxiTypesList" :value="item.codeIndex" :key="item.codeIndex">{{ item.indexName }}</option>
                              </select>
                          </div>
                      </div>
                        ...
                        <div class="article-list">
                            <ul class="after">
                                <li v-for="(item,index) in dataList" v-bind:key="index">
                                    <a target="_self" :href="'../youhuixinxi/detail.jsp?id='+item.id">
                                        <img width="180" class="fl" :src="item.youhuixinxiPhoto" />
                                        <h5>{{item.youhuixinxiName}}</h5>
                                        <p class="time-sour">
                                            <span>发布时间：{{item.insertTime}} </span>
                                        </p>
                                        <p class="reamk">{{item.youhuixinxiContent|subString}}</p>
                                    </a>
                                </li>
                            </ul>
                            <div class="pages">
...
<script>
    var vue = new Vue({
        ...
        data: {
            ...
            dataList: [],//数据
        // 当前表字典表数据
            youhuixinxiTypesList : [],
            youhuixinxiTypes: 0,//类型
            limit: 8,
    ...
    // 当前表的字典表
        ...
        function youhuixinxiTypesSelect() {
            http.request("dictionary/page?page=1&limit=100&sort=&order=&dicCode=youhuixinxi_types", 'get', {}, function (res) {
                if(res.code == 0){
                    vue.youhuixinxiTypesList = res.data.list;
                }
            });
        }
        ...
        function pageList() {
            ...
            if (jquery('#youhuixinxiName').val() != null && jquery('#youhuixinxiName').val() != "" && jquery('#youhuixinxiName').val() != "null" ) {
                param['youhuixinxiName'] = jquery('#youhuixinxiName').val();
            }
                 
            var youhuixinxiTypes = $("#youhuixinxiTypes").find("option:selected").val();
            if (youhuixinxiTypes != null && youhuixinxiTypes != "" && youhuixinxiTypes != "null") {
                param['youhuixinxiTypes'] = youhuixinxiTypes;
            }
                        

            // 获取列表数据
            http.request('youhuixinxi/list', 'get', param, function(res) {
