                ...
                <fieldset class="search-container" style="text-align: center;">
                    <form class="layui-form" lay-filter="myForm">
                          ...
                          <div class="layui-inline" style="margin-bottom: 10px;">
                              ...
                              <div class="layui-input-inline">
                                  <input type="text" name="kechengUuidNumber" id="kechengUuidNumber" placeholder="课程编号" autocomplete="off" class="layui-input">
                              </div>
                          </div>
                    ...
                    <section style="margin: 10px">
                        <div class="row">
                            <div v-for="(item,index) in dataList" class="col-md-3">
                                <i @click="deleteKechengCollection(item.id)" class="layui-icon" style="float:right;color:red;margin-top: 0px;margin-right: 3px;font-size:20px">&#xe640;</i>
                                <div class="gallery-demo" @click="jumpCheck('../kecheng/detail.jsp?id='+item.kechengId , item.kechengDelete == null?'':item.kechengDelete , item.shangxiaTypes == null?'':item.shangxiaTypes)">
                                    <img :src="item.kechengPhoto?item.kechengPhoto.split(',')[0]:''" style="height: 350px;width:100%;"/>
                                    <h4 class="p-mask">{{item.kechengName}}
                                        <span v-if="item.kechengNewMoney"> - ￥{{item.kechengNewMoney}}</span>
                                        <span v-else-if="item.kechengTypes"> - {{item.kechengValue}}</span>
                                    </h4>
                                </div>
                            </div>
                        </div>
...
<script>
    ...
    // 当前表的字典表
        ...
        function kechengCollectionTypesSelect() {
            http.request("dictionary/page?page=1&limit=100&sort=&order=&dicCode=kecheng_collection_types", 'get', {}, function (res) {
                if(res.code == 0){
                    vue.kechengCollectionTypesList = res.data.list;
                }
            });
        }
    ...
    // 级联表的字段数据
        ...
        function pageList() {
            ...
            if (jquery('#kechengUuidNumber').val() != null && jquery('#kechengUuidNumber').val() != "" && jquery('#kechengUuidNumber').val() != "null" ) {
                param['kechengUuidNumber'] = jquery('#kechengUuidNumber').val();
            }
            ...
            //收藏只能查类型为1的
            param['kechengCollectionTypes'] = 1;

            // 获取列表数据
            http.request('kechengCollection/page', 'get', param, function(res) {
