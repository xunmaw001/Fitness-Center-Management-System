                    ...
                    <form class="layui-form" lay-filter="myForm">
                          ...
                          <div class="layui-inline" style="margin-bottom: 10px;">
                              ...
                              <div class="layui-input-inline">
                                  <input type="text" name="qicaiName" id="qicaiName" placeholder="器材名称" autocomplete="off" class="layui-input">
                              </div>
                          </div>
                          ...
                          <div class="layui-inline" style="margin-bottom: 10px;">
                              ...
                              <div class="layui-input-inline">
                                  <input type="text" name="qicaiUuidNumber" id="qicaiUuidNumber" placeholder="器材编号" autocomplete="off" class="layui-input">
                              </div>
                          </div>
                          ...
                          <div class="layui-inline" style="margin-bottom: 10px;">
                              ...
                              <div class="layui-input-inline">
                                  <select name="qicaiTypes" id="qicaiTypes" lay-filter="qicaiTypes">
                                      <option value="">--请选择--</option>
                                      <option v-for="item in qicaiTypesList" :value="item.codeIndex" :key="item.codeIndex">{{ item.indexName }}</option>
                                  </select>
                              </div>
                          </div>
                    ...
                    <section style="margin: 10px">
                        <div class="row">
                            <div v-for="(item,index) in dataList" class="col-md-3">
                                <i @click="deleteQicaiCollection(item.id)" class="layui-icon" style="float:right;color:red;margin-top: 0px;margin-right: 3px;font-size:20px">&#xe640;</i>
                                <div class="gallery-demo" @click="jumpCheck('../qicai/detail.jsp?id='+item.qicaiId , item.qicaiDelete == null?'':item.qicaiDelete , item.shangxiaTypes == null?'':item.shangxiaTypes)">
                                    <img :src="item.qicaiPhoto?item.qicaiPhoto.split(',')[0]:''" style="height: 350px;width:100%;"/>
                                    <h4 class="p-mask">{{item.qicaiName}}
                                        <span v-if="item.qicaiNewMoney"> - ￥{{item.qicaiNewMoney}}</span>
                                        <span v-else-if="item.qicaiTypes"> - {{item.qicaiValue}}</span>
                                    </h4>
                                </div>
                            </div>
                        </div>
...
<script>
    var vue = new Vue({
        ...
        data: {
            ...
            dataList: [],//数据
        // 当前表字典表数据
            qicaiCollectionTypesList : [],
        // 级联表字典表数据
            qicaiTypesList : [],
            shangxiaTypesList : [],
            qicaiCollectionTypes: 0,//类型
            limit: 8,
        ...
        methods: {
            ...
            ,deleteQicaiCollection(id){
                layui.layer.confirm('确认要删除这个收藏吗？', {
                    btn : [ '确定', '取消' ]//按钮
                }, function(index) {
                    var paramArray = [];
                    paramArray.push(id);
                    layui.http.requestJson('qicaiCollection/delete', 'post',paramArray, function (res) {
                        if (res.code == 0) {
                            layer.msg('删除成功', {
                                time: 2000,
                                icon: 6
    ...
    layui.use(['layer', 'element', 'carousel', 'laypage','form', 'http', 'jquery', 'laydate', 'tinymce'], function() {
        ...
        // 查询当前字典表相关
            // 类型的查询和初始化
        qicaiCollectionTypesSelect();
    // 当前表的字典表
        // 类型的查询
        function qicaiCollectionTypesSelect() {
            http.request("dictionary/page?page=1&limit=100&sort=&order=&dicCode=qicai_collection_types", 'get', {}, function (res) {
                if(res.code == 0){
                    vue.qicaiCollectionTypesList = res.data.list;
                }
            });
        }
    ...
    // 级联表的字段数据
        ...
        function qicaiTypesSelect() {
            http.request("dictionary/page?page=1&limit=100&sort=&order=&dicCode=qicai_types", 'get', {}, function (res) {
                if(res.code == 0){
                    vue.qicaiTypesList = res.data.list;
                }
            });
        }
        ...
        function pageList() {
            ...
            if (jquery('#qicaiName').val() != null && jquery('#qicaiName').val() != "" && jquery('#qicaiName').val() != "null" ) {
                param['qicaiName'] = jquery('#qicaiName').val();
            }
                     
            if (jquery('#qicaiUuidNumber').val() != null && jquery('#qicaiUuidNumber').val() != "" && jquery('#qicaiUuidNumber').val() != "null" ) {
                param['qicaiUuidNumber'] = jquery('#qicaiUuidNumber').val();
            }
                                         
            var qicaiTypes = $("#qicaiTypes").find("option:selected").val();
            if (qicaiTypes != null && qicaiTypes != "" && qicaiTypes != "null") {
                            param['qicaiTypes'] = qicaiTypes;
                        }
                                                                                                                                                                
            // 当前表
                                                
            //收藏只能查类型为1的
            param['qicaiCollectionTypes'] = 1;

            // 获取列表数据
            http.request('qicaiCollection/page', 'get', param, function(res) {
