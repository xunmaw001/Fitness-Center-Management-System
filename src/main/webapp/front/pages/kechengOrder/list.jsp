...
<div id="app">
    ...
    <div class="line-container">
        <p class="line" style="background: #EEEEEE;"> 我的课程预约 </p>
    </div>
    <!-- 标题 -->
        ...
        <div class="right-container" style="padding-top: 0;">
            <div class="layui-tab layui-tab-brief" lay-filter="docDemoTabBrief">
                <ul class="layui-tab-title">
                    <li class="layui-this" @click="pageList(0)">全部课程预约</li>
                    <li v-for="item in kechengOrderTypesList" :value="item.codeIndex"  @click="pageList(item.codeIndex)">{{item.indexName}}</li>
                </ul>
                <div class="layui-tab-content">
                    <table class="layui-table" lay-skin="nob">
                        ...
                        <tr v-for="(item,index) in dataList" v-bind:key="index">
                            ...
                            <td style="width: 150px;">{{item.kechengOrderUuidNumber}}</td>
                            <td style="width: 150px;">{{item.kechengOrderTruePrice}}</td>
                            <td style="width: 80px;">{{item.kechengOrderValue}}</td>
                            <td style="width: 150px;">{{item.insertTime}}</td>
                            <td style="width: 100px;">
                                <button v-if="item.kechengOrderTypes==1" @click="refund(item.id)" type="button" class="layui-btn layui-btn-sm layui-btn-radius layui-btn-warm">
...
<script>
    var vue = new Vue({
        ...
        methods: {
            ...
            //列表查询
            ,pageList(kechengOrderTypes) {
                this.kechengOrderTypes = kechengOrderTypes;
                // 获取列表数据
                layui.http.request('kechengOrder/page', 'get', {
                    page: 1,
                    limit: this.limit,
                    kechengOrderTypes: (vue.kechengOrderTypes == 0)?null:vue.kechengOrderTypes
                }, function(res) {
                        ...
                        jump: function(obj, first) {
                            ...
                            if (!first) {
                                layui.http.request('kechengOrder/page', 'get', {
                                    page: obj.curr,
                                    limit: obj.limit,
                                    kechengOrderTypes: (this.kechengOrderTypes == 0)?null:this.kechengOrderTypes
                                }, function(res) {
            ...
            ,pay(id) {
                ...
                // 获取商品详情信息
                layui.http.request(`kechengOrder/pay?id=`+id, 'GET',{}, (res) => {
                    if(res.code==0){
                        layer.msg('支付成功', {
                            time: 2000,
                            icon: 6
            ...
            ,refund(id) {
                ...
                // 获取商品详情信息
                layui.http.request(`kechengOrder/refund?id=`+id, 'get', {}, (res) => {
                    if(res.code==0){
                        layer.msg('退款成功', {
                            time: 2000,
                            icon: 6
            ...
            ,receiving(id) {
                ...
                // 获取商品详情信息
                layui.http.request(`kechengOrder/receiving?id=`+id, 'get', {}, (res) => {
                    if(res.code==0){
                        layer.msg('成功收货', {
                            time: 2000,
                            icon: 6
            ...
            ,commentback(id) {
                $("#id").val(id);//设置订单id
                $('#kechengCommentbackModal').modal('show');//打开模态框
            }
            ,submitKechengCommentback(){
                var kechengCommentbackContent = $("#kechengCommentbackContent").val();
                var id = $("#id").val();

                layui.http.request("kechengOrder/commentback?id="+id+"&commentbackText="+kechengCommentbackContent, 'get', {}, function (res) {
                    if(res.code==0){
                        layer.msg('评价成功', {
                            time: 2000,
                            icon: 6
    ...
    // 当前表的字典表
        ...
        function kechengOrderTypesSelect() {
            http.request("dictionary/page?page=1&limit=100&sort=&order=&dicCode=kecheng_order_types", 'get', {}, function (res) {
                if(res.code == 0){
        ...
        // 获取列表数据
        http.request('kechengOrder/page', 'get',{
            page: 1,
            limit: limit,
            kechengOrderTypes: (vue.kechengOrderTypes == 0)?null:vue.kechengOrderTypes
        }, function(res) {
            ...
            laypage.render({
                ...
                jump: function(obj, first) {
                    ...
                    if (!first) {
                        http.request('kechengOrder/list', 'get', {
                            page: obj.curr,
                            limit: obj.limit,
                            kechengOrderType: (vue.kechengOrderTypes == 0)?null:vue.kechengOrderTypes
                        }, function(res) {
                            vue.dataList = res.data.list
                        })
                    }
