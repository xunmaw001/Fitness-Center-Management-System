...
<div id="app">
    ...
    <div class="line-container">
        <p class="line" style="background: #EEEEEE;"> 我的器材租赁 </p>
    </div>
    <!-- 标题 -->
        ...
        <div class="right-container" style="padding-top: 0;">
            <div class="layui-tab layui-tab-brief" lay-filter="docDemoTabBrief">
                <ul class="layui-tab-title">
                    <li class="layui-this" @click="pageList(0)">全部器材租赁</li>
                    <li v-for="item in qicaiOrderTypesList" :value="item.codeIndex"  @click="pageList(item.codeIndex)">{{item.indexName}}</li>
                </ul>
                <div class="layui-tab-content">
                    <table class="layui-table" lay-skin="nob">
                        ...
                        <tr v-for="(item,index) in dataList" v-bind:key="index">
                            <td style="width: 60px;">{{item.huiyuanName}}</td>
                            <td style="width: 60px;">{{item.qicaiName}}</td>
                            <td style="width: 250px;">
                                <img :src="item.qicaiPhoto" style="width: 100px;height: 100px;object-fit: cover;">
                            </td>
                            <td style="width: 80px;">{{item.qicaiValue}}</td>
                            <td style="width: 150px;">{{item.qicaiOrderUuidNumber}}</td>
                            <td style="width: 150px;">{{item.insertTime}}</td>
                            <td style="width: 150px;">{{item.guihuanTime}}</td>
                            <td style="width: 150px;">{{item.qicaiZulinTian}}</td>
                            <td style="width: 150px;">{{item.qicaiOrderTruePrice}}</td>
                            <td style="width: 80px;">{{item.qicaiOrderValue}}</td>
                            <td style="width: 100px;">
                                <button v-if="item.qicaiOrderTypes==1" @click="refund(item.id)" type="button" class="layui-btn layui-btn-sm layui-btn-radius layui-btn-warm">
                                ...
                                <button v-if="item.qicaiOrderTypes==3" @click="receiving(item.id)" type="button" class="layui-btn layui-btn-sm layui-btn-radius layui-btn-warm">
                                    <i class="layui-icon">&#xe65e;</i> 归还
                                </button>
...
<script>
    var vue = new Vue({
        ...
        methods: {
            ...
            //列表查询
            ,pageList(qicaiOrderTypes) {
                this.qicaiOrderTypes = qicaiOrderTypes;
                // 获取列表数据
                layui.http.request('qicaiOrder/page', 'get', {
                    page: 1,
                    limit: this.limit,
                    qicaiOrderTypes: (vue.qicaiOrderTypes == 0)?null:vue.qicaiOrderTypes
                }, function(res) {
                        ...
                        jump: function(obj, first) {
                            ...
                            if (!first) {
                                layui.http.request('qicaiOrder/page', 'get', {
                                    page: obj.curr,
                                    limit: obj.limit,
                                    qicaiOrderTypes: (this.qicaiOrderTypes == 0)?null:this.qicaiOrderTypes
                                }, function(res) {
            ...
            ,pay(id) {
                ...
                // 获取商品详情信息
                layui.http.request(`qicaiOrder/pay?id=`+id, 'GET',{}, (res) => {
                    if(res.code==0){
                        layer.msg('支付成功', {
                            time: 2000,
                            icon: 6
            ...
            ,refund(id) {
                ...
                // 获取商品详情信息
                layui.http.request(`qicaiOrder/refund?id=`+id, 'get', {}, (res) => {
                    if(res.code==0){
                        layer.msg('退款成功', {
                            time: 2000,
                            icon: 6
            ...
            ,receiving(id) {
                ...
                // 获取商品详情信息
                layui.http.request(`qicaiOrder/receiving?id=`+id, 'get', {}, (res) => {
                    if(res.code==0){
                        layer.msg('成功归还', {
                            time: 2000,
                            icon: 6
            ...
            ,commentback(id) {
                $("#id").val(id);//设置订单id
                $('#qicaiCommentbackModal').modal('show');//打开模态框
            }
            ,submitQicaiCommentback(){
                var qicaiCommentbackContent = $("#qicaiCommentbackContent").val();
                var id = $("#id").val();

                layui.http.request("qicaiOrder/commentback?id="+id+"&commentbackText="+qicaiCommentbackContent, 'get', {}, function (res) {
                    if(res.code==0){
                        layer.msg('评价成功', {
                            time: 2000,
                            icon: 6
    ...
    // 当前表的字典表
        ...
        function qicaiOrderTypesSelect() {
            http.request("dictionary/page?page=1&limit=100&sort=&order=&dicCode=qicai_order_types", 'get', {}, function (res) {
                if(res.code == 0){
        ...
        // 获取列表数据
        http.request('qicaiOrder/page', 'get',{
            page: 1,
            limit: limit,
            qicaiOrderTypes: (vue.qicaiOrderTypes == 0)?null:vue.qicaiOrderTypes
        }, function(res) {
            ...
            laypage.render({
                ...
                jump: function(obj, first) {
                    ...
                    if (!first) {
                        http.request('qicaiOrder/list', 'get', {
                            page: obj.curr,
                            limit: obj.limit,
                            qicaiOrderType: (vue.qicaiOrderTypes == 0)?null:vue.qicaiOrderTypes
                        }, function(res) {
                            vue.dataList = res.data.list
                        })
                    }
