		...
		<script>
			var vue = new Vue({
				...
				computed: {
					totalMoney: function() {
						...
						for (var item of this.dataList) {
							total += item.qicaiNewMoney * item.buyNumber
						}
						return total;
					}
				},
				methods: {
					...
					,payClick() {
						let data={
                            qicais:localStorage.getItem('qicais'),
                            huiyuanId: localStorage.getItem('userid'),
						}
                        // 获取商品详情信息
                        layui.http.request(`qicaiOrder/order`, 'POST', data, (res) => {
                            // 订单编号
                            var orderId = genTradeNo();
							if(res.code == 0){
                                alert("下单成功，点击确定后跳转 我的订单页面");
                                window.location.href='../qicaiOrder/list.jsp';
							}
                       		 window.location.href='../qicaiOrder/list.jsp';
					    });
					}
			...
			layui.use(['layer', 'element', 'carousel', 'http', 'jquery', 'form', 'upload'], function() {
                ...
                // 获取器材购买的清单列表
				var qicais = localStorage.getItem('qicais');
				// 转换成json类型，localstorage保存的是string数据
				vue.dataList = JSON.parse(qicais);
