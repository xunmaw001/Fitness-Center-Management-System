                ...
                <fieldset class="search-container" style="text-align: center;">
                    <form class="layui-form" lay-filter="myForm">
                      ...
                      <div class="layui-inline" style="margin-bottom: 10px;">
                          <label class="layui-form-label">健身资讯名称</label>
                          <div class="layui-input-inline">
                              <input type="text" name="zixunName" id="zixunName" placeholder="健身资讯名称" autocomplete="off" class="layui-input">
                          </div>
                      </div>
         
                      <div class="layui-inline" style="margin-bottom: 10px;">
                          <label class="layui-form-label">健身资讯类型</label>
                          <div class="layui-input-inline">
                              <select name="zixunTypes" id="zixunTypes" lay-filter="zixunTypes">
                                  <option value="">--请选择--</option>
                                  <option v-for="item in zixunTypesList" :value="item.codeIndex" :key="item.codeIndex">{{ item.indexName }}</option>
                              </select>
                          </div>
                      </div>
                        ...
                        <div class="article-list">
                            <ul class="after">
                                <li v-for="(item,index) in dataList" v-bind:key="index">
                                    <a target="_self" :href="'../zixun/detail.jsp?id='+item.id">
                                        <img width="180" class="fl" :src="item.zixunPhoto" />
                                        <h5>{{item.zixunName}}</h5>
                                        <p class="time-sour">
                                            <span>发布时间：{{item.insertTime}} </span>
                                        </p>
                                        <p class="reamk">{{item.zixunContent|subString}}</p>
                                    </a>
                                </li>
                            </ul>
                            <div class="pages">
    ...
    // 当前表的字典表
        ...
        function zixunTypesSelect() {
            http.request("dictionary/page?page=1&limit=100&sort=&order=&dicCode=zixun_types", 'get', {}, function (res) {
                if(res.code == 0){
                    vue.zixunTypesList = res.data.list;
                }
            });
        }
        ...
        function pageList() {
            ...
            if (jquery('#zixunName').val() != null && jquery('#zixunName').val() != "" && jquery('#zixunName').val() != "null" ) {
                param['zixunName'] = jquery('#zixunName').val();
            }
                 
            var zixunTypes = $("#zixunTypes").find("option:selected").val();
            if (zixunTypes != null && zixunTypes != "" && zixunTypes != "null") {
                param['zixunTypes'] = zixunTypes;
            }
