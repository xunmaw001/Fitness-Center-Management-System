                    ...
                    <form class="layui-form" lay-filter="myForm">
                          ...
                          <div class="layui-inline" style="margin-bottom: 10px;">
                              ...
                              <div class="layui-input-inline">
                                  <input type="text" name="changdiName" id="changdiName" placeholder="场地名称" autocomplete="off" class="layui-input">
                              </div>
                          </div>
                          ...
                          <div class="layui-inline" style="margin-bottom: 10px;">
                              ...
                              <div class="layui-input-inline">
                                  <input type="text" name="changdiUuidNumber" id="changdiUuidNumber" placeholder="场地编号" autocomplete="off" class="layui-input">
                              </div>
                          </div>
                          ...
                          <div class="layui-inline" style="margin-bottom: 10px;">
                              ...
                              <div class="layui-input-inline">
                                  <select name="changdiTypes" id="changdiTypes" lay-filter="changdiTypes">
                                      <option value="">--请选择--</option>
                                      <option v-for="item in changdiTypesList" :value="item.codeIndex" :key="item.codeIndex">{{ item.indexName }}</option>
                                  </select>
                              </div>
                          </div>
                    ...
                    <section style="margin: 10px">
                        <div class="row">
                            <div v-for="(item,index) in dataList" class="col-md-3">
                                <i @click="deleteChangdiCollection(item.id)" class="layui-icon" style="float:right;color:red;margin-top: 0px;margin-right: 3px;font-size:20px">&#xe640;</i>
                                <div class="gallery-demo" @click="jumpCheck('../changdi/detail.jsp?id='+item.changdiId , item.changdiDelete == null?'':item.changdiDelete , item.shangxiaTypes == null?'':item.shangxiaTypes)">
                                    <img :src="item.changdiPhoto?item.changdiPhoto.split(',')[0]:''" style="height: 350px;width:100%;"/>
                                    <h4 class="p-mask">{{item.changdiName}}
                                        <span v-if="item.changdiNewMoney"> - ￥{{item.changdiNewMoney}}</span>
                                        <span v-else-if="item.changdiTypes"> - {{item.changdiValue}}</span>
                                    </h4>
                                </div>
                            </div>
                        </div>
...
<script>
    var vue = new Vue({
        ...
        data: {
            ...
            dataList: [],//数据
        // 当前表字典表数据
            changdiCollectionTypesList : [],
        // 级联表字典表数据
            changdiTypesList : [],
            shangxiaTypesList : [],
            changdiCollectionTypes: 0,//类型
            limit: 8,
        ...
        methods: {
            ...
            ,deleteChangdiCollection(id){
                layui.layer.confirm('确认要删除这个收藏吗？', {
                    btn : [ '确定', '取消' ]//按钮
                }, function(index) {
                    var paramArray = [];
                    paramArray.push(id);
                    layui.http.requestJson('changdiCollection/delete', 'post',paramArray, function (res) {
                        if (res.code == 0) {
                            layer.msg('删除成功', {
                                time: 2000,
                                icon: 6
    ...
    layui.use(['layer', 'element', 'carousel', 'laypage','form', 'http', 'jquery', 'laydate', 'tinymce'], function() {
        ...
        // 查询当前字典表相关
            // 类型的查询和初始化
        changdiCollectionTypesSelect();
    // 当前表的字典表
        // 类型的查询
        function changdiCollectionTypesSelect() {
            http.request("dictionary/page?page=1&limit=100&sort=&order=&dicCode=changdi_collection_types", 'get', {}, function (res) {
                if(res.code == 0){
                    vue.changdiCollectionTypesList = res.data.list;
                }
            });
        }
    ...
    // 级联表的字段数据
        ...
        function changdiTypesSelect() {
            http.request("dictionary/page?page=1&limit=100&sort=&order=&dicCode=changdi_types", 'get', {}, function (res) {
                if(res.code == 0){
                    vue.changdiTypesList = res.data.list;
                }
            });
        }
        ...
        function pageList() {
            ...
            if (jquery('#changdiName').val() != null && jquery('#changdiName').val() != "" && jquery('#changdiName').val() != "null" ) {
                param['changdiName'] = jquery('#changdiName').val();
            }
                     
            if (jquery('#changdiUuidNumber').val() != null && jquery('#changdiUuidNumber').val() != "" && jquery('#changdiUuidNumber').val() != "null" ) {
                param['changdiUuidNumber'] = jquery('#changdiUuidNumber').val();
            }
                                         
            var changdiTypes = $("#changdiTypes").find("option:selected").val();
            if (changdiTypes != null && changdiTypes != "" && changdiTypes != "null") {
                            param['changdiTypes'] = changdiTypes;
                        }
                                                                                                                        
            // 当前表
                                                
            //收藏只能查类型为1的
            param['changdiCollectionTypes'] = 1;

            // 获取列表数据
            http.request('changdiCollection/page', 'get', param, function(res) {
