    ...
    <div id="content">
        ...
        <div class="container mt-0">
            <div class="row breadcrumb-bar">
                <div class="col-md-6">
                    <h3 class="block-title">编辑器材收藏</h3>
                </div>
                <div class="col-md-6">
                    <ol class="breadcrumb">
                        ...
                        <li class="breadcrumb-item">器材收藏管理</li>
                        <li class="breadcrumb-item active">编辑器材收藏</li>
                    </ol>
                </div>
            </div>
        </div>
            ...
            <div class="row">
                ...
                <div class="col-md-12">
                    <div class="widget-area-2 lochana-box-shadow">
                        <h3 class="widget-title">器材收藏信息</h3>
                        <form id="addOrUpdateForm">
                            ...
                            <!-- 当前表的字段 -->
                                ...
                                <input id="huiyuanId" name="huiyuanId" type="hidden">
                                    <div class="form-group col-md-6 qicaiCollectionTypesDiv">
                                        <label>类型</label>
                                        <select style="width: 450px" id="qicaiCollectionTypesSelect" name="qicaiCollectionTypes" class="form-control">
                                        </select>
                                    </div>
...
<script>
    ...
    // 下拉框数组
        <!-- 当前表的下拉框数组 -->
    var qicaiCollectionTypesOptions = [];
        <!-- 级联表的下拉框数组 -->
    var huiyuanOptions = [];
    var qicaiOptions = [];
    ...
    function submit() {
        if (validform() == true && compare() == true) {
            ...
            httpJson("qicaiCollection/" + urlParam, "POST", data, (res) => {
                if(res.code == 0){
                    window.sessionStorage.removeItem('addqicaiCollection');
    ...
    // 查询列表
        ...
        function qicaiCollectionTypesSelect() {
            //填充下拉框选项
            http("dictionary/page?page=1&limit=100&sort=&order=&dicCode=qicai_collection_types", "GET", {}, (res) => {
                if(res.code == 0){
                    qicaiCollectionTypesOptions = res.data.list;
                }
            });
        }
        <!-- 查询级联表的所有列表 -->
    ...
    // 初始化下拉框
    <!-- 初始化当前表的下拉框 -->
        function initializationQicaicollectiontypesSelect(){
            var qicaiCollectionTypesSelect = document.getElementById('qicaiCollectionTypesSelect');
            if(qicaiCollectionTypesSelect != null && qicaiCollectionTypesOptions != null  && qicaiCollectionTypesOptions.length > 0 ){
                for (var i = 0; i < qicaiCollectionTypesOptions.length; i++) {
                    qicaiCollectionTypesSelect.add(new Option(qicaiCollectionTypesOptions[i].indexName,qicaiCollectionTypesOptions[i].codeIndex));
                }
            }
        }
    ...
    function setSelectOption() {
        ...
        if(qicaiCollectionTypesSelect != null && qicaiCollectionTypesOptions != null  && qicaiCollectionTypesOptions.length > 0 ) {
            for (var i = 0; i < qicaiCollectionTypesOptions.length; i++) {
                if (qicaiCollectionTypesOptions[i].codeIndex == ruleForm.qicaiCollectionTypes) {//下拉框value对比,如果一致就赋值汉字
                        qicaiCollectionTypesSelect.options[i].selected = true;
                }
            }
        }
        <!--  级联表的下拉框回显  -->
    ...
    function validform() {
        return $("#addOrUpdateForm").validate({
            rules: {
                qicaiId: "required",
                huiyuanId: "required",
                qicaiCollectionTypes: "required",
            },
            messages: {
                qicaiId: "器材不能为空",
                huiyuanId: "会员不能为空",
                qicaiCollectionTypes: "类型不能为空",
            }
        }).form();
    }
    ...
    // 获取当前详情
    function getDetails() {
        var addqicaiCollection = window.sessionStorage.getItem("addqicaiCollection");
        if (addqicaiCollection != null && addqicaiCollection != "" && addqicaiCollection != "null") {
            //注册表单验证
            $(validform());
            ...
            window.sessionStorage.removeItem('updateId');
            http("qicaiCollection/info/" + updateId, "GET", {}, (res) => {
                if(res.code == 0)
                {
                    ruleForm = res.data
                    // 是/否下拉框回显
    ...
    $(document).ready(function () {
        ...
        // 初始化下拉框
            <!--  初始化当前表的下拉框  -->
            initializationQicaicollectiontypesSelect();
            <!--  初始化级联表的下拉框  -->
