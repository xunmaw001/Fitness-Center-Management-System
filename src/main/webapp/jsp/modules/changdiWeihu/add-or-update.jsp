    ...
    <div id="content">
        ...
        <div class="container mt-0">
            <div class="row breadcrumb-bar">
                <div class="col-md-6">
                    <h3 class="block-title">编辑场地维护</h3>
                </div>
                <div class="col-md-6">
                    <ol class="breadcrumb">
                        ...
                        <li class="breadcrumb-item active">编辑场地维护</li>
                    </ol>
                </div>
            </div>
        </div>
            ...
            <div class="row">
                ...
                <div class="col-md-12">
                    <div class="widget-area-2 lochana-box-shadow">
                        <h3 class="widget-title">场地维护信息</h3>
                        <form id="addOrUpdateForm">
                            ...
                            <!-- 当前表的字段 -->
                                ...
                                <input id="yuangongId" name="yuangongId" type="hidden">
                                    <div class="form-group col-md-6 changdiWeihuUuidNumberDiv">
                                        <label>维护编号</label>
                                        <input style="width: 450px" id="changdiWeihuUuidNumber" name="changdiWeihuUuidNumber" class="form-control"
                                               placeholder="维护编号">
                                    </div>
                                    <div class="form-group col-md-6 changdiWeihuTypesDiv">
                                        <label>场地维护类型</label>
                                        <select style="width: 450px" id="changdiWeihuTypesSelect" name="changdiWeihuTypes" class="form-control">
                                        </select>
                                    </div>
                                    <div class="form-group  col-md-6 changdiWeihuContentDiv">
                                        <label>场地维护详情</label>
                                        <input id="changdiWeihuContentupload" name="file" type="file">
                                        <script id="changdiWeihuContentEditor" type="text/plain"
                                                style="width:100%;height:230px;"></script>
                                        <script type = "text/javascript" >
                                        //建议使用工厂方法getEditor创建和引用编辑器实例，如果在某个闭包下引用该编辑器，直接调用UE.getEditor('editor')就能拿到相关的实例
                                        //相见文档配置属于你自己的编译器
                                        var changdiWeihuContentUe = UE.getEditor('changdiWeihuContentEditor', {
                                            toolbars: [
                                                [
                                                    'undo', //撤销
                                                    'bold', //加粗
                                        ...
                                        <input type="hidden" name="changdiWeihuContent" id="changdiWeihuContent-input">
                                    </div>
...
<script>
    ...
    function upload() {
        ...
        $('#changdiWeihuContentupload').fileupload({
            url: baseUrl + 'file/upload',
            headers: {token: window.sessionStorage.getItem("token")},
            dataType: 'json',
            done: function (e, data) {
                UE.getEditor('changdiWeihuContentEditor').execCommand('insertHtml', '<img src=\"' + baseUrl + 'upload/' + data.result.file + '\" width=900 height=560>');
            }
        });
    ...
    function submit() {
        if (validform() == true && compare() == true) {
            ...
            httpJson("changdiWeihu/" + urlParam, "POST", data, (res) => {
                if(res.code == 0){
                    window.sessionStorage.removeItem('addchangdiWeihu');
    ...
    // 初始化下拉框
    <!-- 初始化当前表的下拉框 -->
        function initializationChangdiweihutypesSelect(){
            var changdiWeihuTypesSelect = document.getElementById('changdiWeihuTypesSelect');
            if(changdiWeihuTypesSelect != null && changdiWeihuTypesOptions != null  && changdiWeihuTypesOptions.length > 0 ){
                for (var i = 0; i < changdiWeihuTypesOptions.length; i++) {
                    changdiWeihuTypesSelect.add(new Option(changdiWeihuTypesOptions[i].indexName,changdiWeihuTypesOptions[i].codeIndex));
                }
            }
        }
    ...
    function setSelectOption() {
        ...
        if(changdiWeihuTypesSelect != null && changdiWeihuTypesOptions != null  && changdiWeihuTypesOptions.length > 0 ) {
            for (var i = 0; i < changdiWeihuTypesOptions.length; i++) {
                if (changdiWeihuTypesOptions[i].codeIndex == ruleForm.changdiWeihuTypes) {//下拉框value对比,如果一致就赋值汉字
                        changdiWeihuTypesSelect.options[i].selected = true;
                }
            }
        }
        <!--  级联表的下拉框回显  -->
    ...
    function setContent() {
        ...
        <!-- 当前表的填充富文本框 -->
        if (ruleForm.changdiWeihuContent != null && ruleForm.changdiWeihuContent != 'null' && ruleForm.changdiWeihuContent != '' && $("#changdiWeihuContentupload").length>0) {

            var changdiWeihuContentUeditor = UE.getEditor('changdiWeihuContentEditor');
            changdiWeihuContentUeditor.ready(function () {
                var mes = '';
                if(ruleForm.changdiWeihuContent != null){
                    mes = ''+ ruleForm.changdiWeihuContent;
                    // mes = mes.replace(/\n/g, "<br>");
                }
                changdiWeihuContentUeditor.setContent(mes);
            });
        }
    }
    // 获取富文本框内容
    function getContent() {
        ...
        <!-- 获取当前表的富文本框内容 -->
        if($("#changdiWeihuContentupload").length>0) {
            var changdiWeihuContentEditor = UE.getEditor('changdiWeihuContentEditor');
            if (changdiWeihuContentEditor.hasContents()) {
                $('#changdiWeihuContent-input').attr('value', changdiWeihuContentEditor.getContent());
            }
        }
    }
    //数字检查
    ...
    function validform() {
        return $("#addOrUpdateForm").validate({
            rules: {
                changdiId: "required",
                yuangongId: "required",
                changdiWeihuUuidNumber: "required",
                changdiWeihuTypes: "required",
                changdiWeihuContent: "required",
                weihuTime: "required",
            },
            messages: {
                changdiId: "场地不能为空",
                yuangongId: "员工不能为空",
                changdiWeihuUuidNumber: "维护编号不能为空",
                changdiWeihuTypes: "场地维护类型不能为空",
                changdiWeihuContent: "场地维护详情不能为空",
                weihuTime: "维护时间不能为空",
            }
        }).form();
    }
    ...
    // 获取当前详情
    function getDetails() {
        var addchangdiWeihu = window.sessionStorage.getItem("addchangdiWeihu");
        if (addchangdiWeihu != null && addchangdiWeihu != "" && addchangdiWeihu != "null") {
            //注册表单验证
            $(validform());
            $("#changdiWeihuUuidNumber").val(new Date().getTime()+Math.ceil(Math.random()*10));//设置唯一号
            ...
            window.sessionStorage.removeItem('updateId');
            http("changdiWeihu/info/" + updateId, "GET", {}, (res) => {
                if(res.code == 0)
                {
                    ruleForm = res.data
                    // 是/否下拉框回显
    ...
    <!--  当前表的数据回显  -->
        ...
        $("#changdiWeihuUuidNumber").val(ruleForm.changdiWeihuUuidNumber);
        $("#changdiWeihuContent").val(ruleForm.changdiWeihuContent);
    ...
    $(document).ready(function () {
        ...
        // 初始化下拉框
            <!--  初始化当前表的下拉框  -->
            initializationChangdiweihutypesSelect();
            <!--  初始化级联表的下拉框  -->
