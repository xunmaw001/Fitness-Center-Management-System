    ...
    <div id="content">
        ...
        <div class="container mt-0">
            <div class="row breadcrumb-bar">
                <div class="col-md-6">
                    <h3 class="block-title">编辑场地收藏</h3>
                </div>
                <div class="col-md-6">
                    <ol class="breadcrumb">
                        ...
                        <li class="breadcrumb-item">场地收藏管理</li>
                        <li class="breadcrumb-item active">编辑场地收藏</li>
                    </ol>
                </div>
            </div>
        </div>
            ...
            <div class="row">
                ...
                <div class="col-md-12">
                    <div class="widget-area-2 lochana-box-shadow">
                        <h3 class="widget-title">场地收藏信息</h3>
                        <form id="addOrUpdateForm">
                            ...
                            <!-- 当前表的字段 -->
                                ...
                                <input id="huiyuanId" name="huiyuanId" type="hidden">
                                    <div class="form-group col-md-6 changdiCollectionTypesDiv">
                                        <label>类型</label>
                                        <select style="width: 450px" id="changdiCollectionTypesSelect" name="changdiCollectionTypes" class="form-control">
                                        </select>
                                    </div>
...
<script>
    ...
    var tableName = "changdiCollection";
    ...
    // 下拉框数组
        <!-- 当前表的下拉框数组 -->
    var changdiCollectionTypesOptions = [];
        <!-- 级联表的下拉框数组 -->
    var changdiOptions = [];
    var huiyuanOptions = [];
    ...
    function submit() {
        if (validform() == true && compare() == true) {
            ...
            httpJson("changdiCollection/" + urlParam, "POST", data, (res) => {
                if(res.code == 0){
                    window.sessionStorage.removeItem('addchangdiCollection');
    ...
    // 查询列表
        ...
        function changdiCollectionTypesSelect() {
            //填充下拉框选项
            http("dictionary/page?page=1&limit=100&sort=&order=&dicCode=changdi_collection_types", "GET", {}, (res) => {
                if(res.code == 0){
                    changdiCollectionTypesOptions = res.data.list;
                }
            });
        }
        <!-- 查询级联表的所有列表 -->
    ...
    // 初始化下拉框
    <!-- 初始化当前表的下拉框 -->
        function initializationChangdicollectiontypesSelect(){
            var changdiCollectionTypesSelect = document.getElementById('changdiCollectionTypesSelect');
            if(changdiCollectionTypesSelect != null && changdiCollectionTypesOptions != null  && changdiCollectionTypesOptions.length > 0 ){
                for (var i = 0; i < changdiCollectionTypesOptions.length; i++) {
                    changdiCollectionTypesSelect.add(new Option(changdiCollectionTypesOptions[i].indexName,changdiCollectionTypesOptions[i].codeIndex));
                }
            }
        }
    ...
    function setSelectOption() {
        ...
        if(changdiCollectionTypesSelect != null && changdiCollectionTypesOptions != null  && changdiCollectionTypesOptions.length > 0 ) {
            for (var i = 0; i < changdiCollectionTypesOptions.length; i++) {
                if (changdiCollectionTypesOptions[i].codeIndex == ruleForm.changdiCollectionTypes) {//下拉框value对比,如果一致就赋值汉字
                        changdiCollectionTypesSelect.options[i].selected = true;
                }
            }
        }
        <!--  级联表的下拉框回显  -->
    ...
    function validform() {
        return $("#addOrUpdateForm").validate({
            rules: {
                changdiId: "required",
                huiyuanId: "required",
                changdiCollectionTypes: "required",
            },
            messages: {
                changdiId: "场地不能为空",
                huiyuanId: "会员不能为空",
                changdiCollectionTypes: "类型不能为空",
            }
        }).form();
    }
    ...
    // 获取当前详情
    function getDetails() {
        var addchangdiCollection = window.sessionStorage.getItem("addchangdiCollection");
        if (addchangdiCollection != null && addchangdiCollection != "" && addchangdiCollection != "null") {
            //注册表单验证
            $(validform());
            ...
            window.sessionStorage.removeItem('updateId');
            http("changdiCollection/info/" + updateId, "GET", {}, (res) => {
                if(res.code == 0)
                {
                    ruleForm = res.data
                    // 是/否下拉框回显
    ...
    $(document).ready(function () {
        ...
        // 初始化下拉框
            <!--  初始化当前表的下拉框  -->
            initializationChangdicollectiontypesSelect();
            <!--  初始化级联表的下拉框  -->
