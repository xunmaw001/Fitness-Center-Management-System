    ...
    <div id="content">
        ...
        <div class="container mt-0">
            <div class="row breadcrumb-bar">
                <div class="col-md-6">
                    <h3 class="block-title">编辑课程预约</h3>
                </div>
                <div class="col-md-6">
                    <ol class="breadcrumb">
                        ...
                        <li class="breadcrumb-item active">编辑课程预约</li>
                    </ol>
                </div>
            </div>
        </div>
            ...
            <div class="row">
                ...
                <div class="col-md-12">
                    <div class="widget-area-2 lochana-box-shadow">
                        <h3 class="widget-title">课程预约信息</h3>
                        <form id="addOrUpdateForm">
                            ...
                            <!-- 当前表的字段 -->
                                    ...
                                    <div class="form-group col-md-6 kechengOrderUuidNumberDiv">
                                        <label>预约流水号</label>
                                        <input style="width: 450px" id="kechengOrderUuidNumber" name="kechengOrderUuidNumber" class="form-control"
                                               placeholder="预约流水号">
                                    </div>
                                <input id="kechengId" name="kechengId" type="hidden">
                                <input id="huiyuanId" name="huiyuanId" type="hidden">
                                    <div class="form-group col-md-6 kechengOrderTruePriceDiv">
                                        <label>实付金额</label>
                                        <input style="width: 450px" id="kechengOrderTruePrice" name="kechengOrderTruePrice" class="form-control"
                                               onchange="kechengOrderTruePriceChickValue(this)" placeholder="实付金额">
                                    </div>
                                    <div class="form-group col-md-6 kechengOrderTypesDiv">
                                        <label>课程预约状态</label>
                                        <select style="width: 450px" id="kechengOrderTypesSelect" name="kechengOrderTypes" class="form-control">
                                        </select>
                                    </div>
...
<script>
    ...
    // 初始化下拉框
    <!-- 初始化当前表的下拉框 -->
        function initializationKechengordertypesSelect(){
            var kechengOrderTypesSelect = document.getElementById('kechengOrderTypesSelect');
            if(kechengOrderTypesSelect != null && kechengOrderTypesOptions != null  && kechengOrderTypesOptions.length > 0 ){
                for (var i = 0; i < kechengOrderTypesOptions.length; i++) {
                    kechengOrderTypesSelect.add(new Option(kechengOrderTypesOptions[i].indexName,kechengOrderTypesOptions[i].codeIndex));
                }
            }
        }
    ...
    function setSelectOption() {
        ...
        if(kechengOrderTypesSelect != null && kechengOrderTypesOptions != null  && kechengOrderTypesOptions.length > 0 ) {
            for (var i = 0; i < kechengOrderTypesOptions.length; i++) {
                if (kechengOrderTypesOptions[i].codeIndex == ruleForm.kechengOrderTypes) {//下拉框value对比,如果一致就赋值汉字
                        kechengOrderTypesSelect.options[i].selected = true;
                }
            }
        }
        <!--  级联表的下拉框回显  -->
    ...
    //数字检查
        <!-- 当前表的数字检查 -->
        function kechengOrderTruePriceChickValue(e){
    ...
    function validform() {
        return $("#addOrUpdateForm").validate({
            rules: {
                kechengOrderUuidNumber: "required",
                kechengId: "required",
                huiyuanId: "required",
                kechengOrderTruePrice: "required",
                kechengOrderTypes: "required",
            },
            messages: {
                kechengOrderUuidNumber: "预约流水号不能为空",
                kechengId: "课程不能为空",
                huiyuanId: "会员不能为空",
                kechengOrderTruePrice: "实付金额不能为空",
                kechengOrderTypes: "课程预约状态不能为空",
            }
        }).form();
    }
    ...
    function getDetails() {
        ...
        if (addkechengOrder != null && addkechengOrder != "" && addkechengOrder != "null") {
            //注册表单验证
            $(validform());
            $("#kechengOrderUuidNumber").val(new Date().getTime()+Math.ceil(Math.random()*10));//设置唯一号
    ...
    <!--  当前表的数据回显  -->
        ...
        $("#kechengOrderUuidNumber").val(ruleForm.kechengOrderUuidNumber);
        $("#kechengId").val(ruleForm.kechengId);
        $("#huiyuanId").val(ruleForm.huiyuanId);
        $("#kechengOrderTruePrice").val(ruleForm.kechengOrderTruePrice);
    ...
    function kechengDataBind(){
        ...
        $("#kechengKaishiTime").val(ruleForm.kechengKaishiTime);
        $("#kechengJieshuTime").val(ruleForm.kechengJieshuTime);
        $("#kechengNumber").val(ruleForm.kechengNumber);
        $("#kechengOldMoney").val(ruleForm.kechengOldMoney);
        $("#kechengNewMoney").val(ruleForm.kechengNewMoney);
        $("#kechengClicknum").val(ruleForm.kechengClicknum);
    ...
    $(document).ready(function () {
        ...
        // 初始化下拉框
            <!--  初始化当前表的下拉框  -->
            initializationKechengordertypesSelect();
            <!--  初始化级联表的下拉框  -->
